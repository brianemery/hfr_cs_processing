...
%
%
% Version modified by Brian Emery:
% - add ancillary data to the OtherMatrixVariables field ('ESPC','ETMP', ...
%   'MAXV','MINV','EDVC','ERTC')
% - Idea: create a substitute for warning function that writes the warning
%   data to some kind of processing log file (that works with parfor)
...
if C > 0
  RADIAL.OtherMetadata(1).loadDataFileWithChecks_CODE = C;
  RADIAL.OtherMetadata.loadDataFileWithChecks_TEXT = tttt;
end


% BE: NEED TESTS FOR THESE
% and fix display output. I have an example of 1/2 a file that causes one
% of these errors, and it could be loaded with other methods (last line is
% short the number of cols)
...
if C >= 1e10 % Improbable that this should occur
  ...
  return
elseif C >=1e3
      % BE NOTE: this msg is also in loadDataFileWithChecks
...
% Get header
[RADIAL.OtherMetadata(1).Header,names,values] = getRDLHeader( filename );
...
if isempty(ii)
  ...
  vv = cellstr(strparser( vv{1} ));
  RADIAL.SiteName = upper(vv{1}); % BE - Standardize on uppercase SiteName
end
...
if isempty(ii)
  ...
  RADIAL.Type = 'RDLIdeal'; %'UNKNOWN'; BE: WERA has no type but are ideal
else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% If data is empty, nothing to do. BE note: this usually means the path is
% incorrect 
if isempty(Data)
  feval( warnfunc, [ mfilename ':NO_DATA' ], ...
         'No Data in file.  Returning empty structure. Check File Name, Path' );
  return
end
...
%
% INPUTS
% vv   - Cell array of TableColumnTypes (keys for column numbers), the
%        order of these names correspond to the columns in 'Data'
% Data - Matrix of data in from the Codar ctf file
% R    - 'OtherMatrixVars' substruct to propagate forward

% Brian Emery, March 2010
%    June 2011 - reworked to get all fields not explicitly retrieved above
...
for m = 1:numel(fn)
        ...
        R.(fn{m}) = Data(:,strcmp(fn{m},vv));
        
        % set 999's to NaN
        R.(fn{m})( R.(fn{m}) == 999 ) = NaN;
